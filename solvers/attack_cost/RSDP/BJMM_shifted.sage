...
def cost_shifted_bjmm(p, z, n, k, l, v0, delta1, delta2, e1, e2, Mmax = -1, verb = False):
    """
    ...
    # additive structure
    global alphaEshift, alphaDshift, zdshift
    #if verb: print("alphaE=",alphaE,"zd=","alphaD=",alphaD)
...
#------------new alpha
    ...
    r0 = binomial(v0, v1)  * binomial(v0 - v1, delta1)**2 * alphaD**(2*delta1) * binomial(v1,2*e1) * alphaE**(2*e1)
    u0 = floor(log(r0,p))
    u0 = N(min(u0,l))
    ...
    r1 = binomial(v1, v2) * binomial(v1 - v2, delta2)**2 * alphaD**(2*delta2) * binomial(delta1, delta1/2) * binomial(v2,2*e2)*alphaE**(2*e2) 
    l1 = floor(log(r1,p))
    l1 = N(min(l1,u0))
    l0 = u0 - l1
...
def opt_shifted_bjmm_old(p, z, n, k, Mmax=-1, verb=True):
    """
    ...
    C_opt = 10**6
    M_opt = 10**6
    P_opt = {'ell': -1, 'v0': -1, 'delta1': -1, 'delta2': -1, 'e1': 0, 'e2': 0}
    for l in range(0, n-k+1):
        for v0 in range(0,k+l+1):
            if v0%2: continue
            for delta1 in range(0, min(N(v0/2), 17)+1): 
                if delta1 > v0 - v1: continue
                d1 = delta1
                if d1%2: continue
                for delta2 in range(0, min(N(v0/4), 9)+1):
                    if delta2 > v1-v2: continue 
                    d2 = d1/2+delta2
                    if d2%2:continue
                    C, M = cost_shifted_bjmm(p, z, n, k, l, v0, delta1, delta2,e1, e2, Mmax, False)
                    if C < C_opt:
                        M_opt       = M
                        C_opt       = C
                        P_opt = {'ell': l, 'v0': v0, 'delta1': delta1, 'delta2': delta2, 'e1': 0, 'e2':0}

    if verb: cost_shifted_bjmm(p, z, n, k, P_opt['ell'], P_opt['v0'], P_opt['delta1'], P_opt['delta2'], P_opt['e1'], P_opt['e2'], Mmax, True)
    return C_opt, M_opt, P_opt


def opt_shifted_bjmm(p, z, n, k, Mmax=-1, verb=True):
    """
    optimize parameters of BJMM solver on shifted instance
    inputs:
    p:      field size
    ...
    for l in range(0, n-k+1):
        for v0 in range(0,k+l+1):
            if v0%8: continue
            for e1 in range(0, min(v0/2, 10) +1):
                v1 = v0/2 + e1
                if v1%2: continue
                for e2 in range(0, min(v1/2,10)+1):
                    v2 = v1/2 + e2
                    if v2%2: continue
                    for delta1 in range(0, min(v0/2, 17)+1): 
                        ...
                        for delta2 in range(0, min(v0/4, 9)+1): 
                            d2 = d1/2+delta2
                            if d2%2:continue
                            C, M = cost_shifted_bjmm(p, z, n, k, l, v0, delta1, delta2,0, 0, Mmax, False)
                            if C < C_opt:
                                M_opt       = M
                                C_opt       = C
                                P_opt = {'ell': l, 'v0': v0, 'delta1': delta1, 'delta2': delta2, 'e1': e1, 'e2':e2}
